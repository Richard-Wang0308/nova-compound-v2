...
async def process_epoch(config, current_block, metagraph, subtensor, wallet):
    ...
    try:
        ...
        # Initialize scoring structure (simplified - no PSICHIC scores)
        score_dict = {
            uid: {
                "entropy": None,
                "entropy_boltz": None,
        ...
        # Determine winner (Boltz only)
        winner_boltz = determine_winner(score_dict)
    ...
    while True:
        try:
            ...
            if current_block % config.epoch_length == 0:
                ...
                winner_boltz = await process_epoch(config, current_block, metagraph, subtensor, wallet)
                if not test_mode:
                    await set_weights(winner_boltz, config)
